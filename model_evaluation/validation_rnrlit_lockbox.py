...
### For models that were trained with connections between specific nodes (from intelligence theories),
### models trained with links (edges,connections) between randomly chosen nodes,
### and models trained with randomly chosen links
...
path_models = 'res_30nodes_connection_rand' #Folder with models 
...
name_score = 'g_score' #Intelligence component
...
#Read subject order of subjects, intelligence scores, confounds in behavioral data / main sample
...
#Read subject order of subjects, intelligence scores, confounds in behavioral data / lockbox sample
...
cnt_models = np.zeros(len(states)) #Number of iterations per state
...
for n_state, state in enumerate(states):
    ...
    n_network = 0 
    name_files = state + '_' + name_score + '_' + str(n_network) + '_' + str(0) + '_' + 'real'
    files_iteration = [filename for filename in os.listdir(path_complete) if filename.startswith(name_files) and not 'parameters' in filename
                  and not 'corr' in filename and not 'idx' in filename and not 'y_pred' in filename and not 'y_test' in filename]
    ...
    for i in IDs_iteration_unique[0:100]:
        ...
        for k in range(n_folds):
            ...
            #Load links (edges) used for prediction
            name_parameters_i = state + '_' + name_score + '_' + str(n_network) + '_' + 'real' + '_' + i
            path_parameters_i = os.path.join(path_complete, name_parameters_i)
            with open(path_parameters_i, "rb") as fp:
                model_parameter_i = pickle.load(fp)
            
            edges = model_parameter_i[-1]
            ...
            #Get FC values of links
            idx_m,idx_n = np.where(mask==1)
...
name_save = 'corr_g_lockbox_' + name_nodes
