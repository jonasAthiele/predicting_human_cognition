...
for sc in range(n_states):
    ...
    #Read FCs of the chosen state
    idx_state = sc + 1
    name_FC = 'FC' + str(idx_state) + '_HCP_610_100nodes.mat'
    path_fc = os.path.join(demo_folder, 'source', name_FC)
    FC_state = spio.loadmat(path_fc)['FC']
    
    #Binarize FC
    FC_mean_bin = np.mean(FC_state,2)
    ...
    mask[idx1, idx2] = np.mean(FC_state[idx1, idx2, :],1)
