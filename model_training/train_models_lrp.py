...
###Training with different numbers of the most relevant edges
...
#Compute attribution of each connection
...
n_nodes_remove = [45,145,245,345,220] #Numbers of connections to remove iteratively
method_relevance = 'lrp' #Method to estimate relevance of connections, choose from: LRP: 'lrp', intgrated gradients: 'ig', feature permutation: 'fp', feature ablation: 'fa'
...
for p_real in range(n_permutations_real):
    ...
    for state in states:
        ...
        for k in range(n_folds):
            ...
            #Number of connections for prediction
            idx_remaining = np.arange(X_train.shape[1])

            X_train_relevance = X_train.copy()
            X_test_relevance = X_test.copy()
            
            #Store indexes of removed connections
            idx_removed_all = []
            
            n_iterations = len(n_nodes_remove)
            
            #Iteratively remove most relevant connections (step-wise relevance estimation)
            ...
            #Train models with different numbers of most relevant connections (connections were identified above)
        ...
        with open(path_complete, "wb") as fp:   
            pickle.dump(ind_removed_relevants, fp) #Indexes of remaining edges in process of removing most relevant edges
